...
def create_app():
    ...
    # Render / Railway / דומיינים שונים: נשתמש ב-host בפועל של הבקשה
    # שים לב: צריך להוסיף את כתובת ה-redirect (למשל https://<host>/auth) בקונסולת ה-OAuth
    def get_redirect_uri():
        ...
        if "yedaarechev.com" in host:
            ...
            # עבור Render (onrender.com) וכל דומיין אחר – בנה URI דינמי מהבקשה
            scheme = request.headers.get("X-Forwarded-Proto", request.scheme) or "https"
            # בלוקאלי נעדיף http כדי לא לשבור
            if "localhost" in host or host.startswith("127.0.0.1"):
                scheme = "http"
            uri = f"{scheme}://{host}/auth"
        print(f"[AUTH] Using redirect_uri={uri} (host={host})")
        return uri

    # Secrets
    db_url = os.environ.get("DATABASE_URL")
    # Render/Railway לעיתים מספקים postgres://, ו-SQLAlchemy מצפה ל-postgresql://
...
if __name__ == '__main__':
    # Render מריץ את השירות עם משתנה PORT חובה, והאפליקציה חייבת להאזין על 0.0.0.0
