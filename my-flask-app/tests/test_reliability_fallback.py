...
def test_search_details_normalizes_old_history_to_information_review(logged_in_client, app):
    ...
    with app.app_context():
        db.session.add(
            main.SearchHistory(
                ...
                fuel_type="בנזין",
                transmission="אוטומטית",
                result_json=json.dumps({"reliability_summary": "ok"}, ensure_ascii=False),
            )
        )
        db.session.commit()
